...
1. Requests for access to Dynaton systems and applications are made formally using the following process:
1. A Dynaton workforce member initiates the access request by filling out a **Systems Access Form** [here](https://docs.google.com/forms/d/e/1FAIpQLSdus2wN2WHjbj0JBal2aG6Z8NmguTP7am1GCmZAgPvb7zSKaw/viewform?usp=sf_link).
...
1. Once the review is completed, the Security Officer or Privacy Officer approves or rejects the form. If the form is rejected, it goes back for further review and documentation.
1. If the review is approved, the Security Officer or Privacy Officer then updates the Systems Access Form as Done, adding any pertinent notes required. The Chief Technology Officer (CTO) then grants requested access.
   ...
   - For non-production systems, access grants are accomplished by leveraging the access control mechanisms built into those systems. Account management for non-production systems may be delegated to a Dynaton employee at the discretion of the Security Officer, Privacy Officer, or CTO.
1. Access is not granted until receipt, review, and approval by the Dynaton Security Officer or Privacy Officer.
1. The request for access is retained for future reference.
1. All access to Dynaton systems and services is reviewed and updated on a bi-annual basis to ensure proper authorizations are in place commensurate with job functions. The form used to conduct account review is on Google Drive. A security consultant is designated by the Security Officer or CTO to perform the access review.
1. Any Dynaton workforce member can request change of access using the process outlined in [§7.2 paragraph 1](#7.2-access-establishment-and-modification).
...
1. In the case of non-personal information, such as generic educational content, identification and authentication may not be required. This is the responsibility of Dynaton Customers to define, and not Dynaton.
...
1. Generic accounts are not allowed on Dynaton systems.
...
3. Dynaton maintains a minimum necessary approach to access to Customer data. As such, Dynaton LLCluding all workforce members, does not readily have access to any ePHI.
...
1. Role based access categories for each Dynaton system and application are pre-approved by the Security Officer, or an authorized delegate of the Security Officer. Defined roles can be referenced in Appendix 1 of the systems access policy.
2. Dynaton utilizes hardware and software firewalls to segment data, prevent unauthorized access, and monitor traffic for denial of service attacks.
...
1. Access to the Dynaton Platform systems and applications is controlled by requiring unique User Login IDs and passwords for each individual user and developer.
2. Passwords requirements mandate strong password controls (see below).
3. Passwords are not displayed at any time and are not transmitted or stored in plain text.
4. Default accounts on all production systems, including root, are disabled.
5. Shared accounts are not allowed within Dynaton systems or networks.
6. Automated log-on configurations that store user passwords or bypass password entry are not permitted for use with Dynaton workstations or production systems.
...
All workstations at Dynaton are company owned, and all are laptop Apple products running Mac OSX or Linux.
...
9. All workstations are to have the following messages added to the lock screen and login screen: _This computer is owned by Dynaton LLC. By logging in, unlocking, and/or using this computer you acknowledge you have seen, and follow, these policies (https://policy.dynatondata.com) and have completed this training (https://training.dynatondata.com/). Please contact us if you have problems with this - privacy@dynatondata.com._
...
1. Dynaton production systems are not accessible directly over wireless channels.
...
4. Wireless networks managed within Dynaton non-production facilities (offices, etc.) are secured with the following configurations:
   - All data in transit over wireless is encrypted using WPA2 encryption;
   - Passwords are rotated on a regular basis, presently quarterly. This process is managed by the Dynaton Security Officer.
...
3. The Security Officer will terminate users' access rights immediately upon notification, and will coordinate with the appropriate Dynaton employees to terminate access to any non-production systems managed by those employees.
...
Dynaton does not use paper records for any sensitive information. Use of paper for recording and storing sensitive data is against Dynaton policies.

## 7.12 Password Management

1. User IDs and passwords are used to control access to Dynaton systems and may not be disclosed to anyone for any reason.
2. Users may not allow anyone, for any reason, to have access to any information system using another user's unique user ID and password.
3. On all production systems and applications in the Dynaton environment, password configurations are set to require:
